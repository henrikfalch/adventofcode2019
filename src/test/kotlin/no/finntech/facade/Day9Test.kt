...
internal class Day9Test {
    ...
    fun part1Example1() {
        ...
        assertEquals(program, day.startIntComputer(program, BigInteger.ONE))
    }

    @Test
    fun part1Example2() {
        val program = listOf(1102,34915192,34915192,7,4,7,99,0).map { it.toBigInteger() }
        assertEquals(program[1]*program[2], day.startIntComputer(program, BigInteger.ONE)[0])
    }

    @Test
    fun part1Example3() {
        val program = listOf(104,1125899906842624,99).map { it.toBigInteger() }
        assertEquals(program[1], day.startIntComputer(program, BigInteger.ONE)[0])
    }
}
